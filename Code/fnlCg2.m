function x = fnlCg2(x0,params,coil_sens,y,k)
%-----------------------------------------------------------------------
%
% res = fnlCg(x0,params)
%
...
g0 = wGradient(x,params,c_sens,kspace,mask);
...
% backtracking line-search
	...
	g1 = wGradient(x,params,c_sens,kspace,mask);
	bk = g1(:)'*g1(:)/(g0(:)'*g0(:)+eps);
	g0 = g1;
	dx =  - g1 + bk* dx;
	k = k + 1;
...
function grad = wGradient(x,params,c_sens,kspace,mask)

gradXFM = 0;
gradTV = 0;

gradObj = gOBJ(x,params,c_sens,kspace,mask);
if params.xfmWeight
...
function gradObj = gOBJ(x,params,c_sens,kspace,mask);
% computes the gradient of the data consistency

% 	gradObj = params.XFM*(params.FT'*(params.FT*(params.XFM'*x) - params.data));
% gradObj = (conj(c_sens).*(param.FT'*((param.FT*(c_sens(:,:,[1:12]).*(param.XFM'*x))) - kspace)));

for i=1:12
one(:,:,i)=c_sens(:,:,i).*(params.XFM'*x);
end
for j=1:12
   two(:,:,j)= params.FT*one(:,:,j);
end

 data_consis=(two.*mask)+kspace;
% data_consis=two-kspace;


for i=1:12
back(:,:,i)=params.FT'*data_consis(:,:,i);
end
for j=1:12
back_last(:,:,j)=conj(c_sens(:,:,j)).*back(:,:,j);
end
