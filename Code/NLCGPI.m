function x = NLCG(x0,params,coil_sens,y,k,Nc)
x = x0;
c_sens=coil_sens;
kspace=y;
mask=k;
...
g0 = wGradient(x,params,c_sens,kspace,mask,Nc);
...
% 	disp(sprintf('%d   , obj: %f, RMS: %f, L-S: %d', k,f1,RMSerr,lsiter));
	...
	g1 = wGradient(x,params,c_sens,kspace,mask,Nc);
	bk = g1(:)'*g1(:)/(g0(:)'*g0(:)+eps);
	g0 = g1;
	dx =  - g1 + bk* dx;
	k = k + 1;
...
function grad = wGradient(x,params,c_sens,kspace,mask,Nc)

gradXFM = 0;
gradTV = 0;

gradObj = gOBJ(x,params,c_sens,kspace,mask,Nc);
if params.xfmWeight
...
function gradObj = gOBJ(x,params,c_sens,kspace,mask,Nc)
